...
let rec e_reg t r =
  ...
  match t with
    ...
    | <LOCAL n> ->
        emit "ldr" [Register R_ip; Literal (nosym, n)];
    ...
    | <BOUND, t1, t2> ->
        ...
        release v2;
        emit "cmp" [v1; v2];
        emit "ldrhs" [Register (R 0); Literal (nosym, !line)];
    ...
    | <NCHECK, t1> ->
        let v1 = e_reg t1 r in
        emit "cmp" [v1; Const 0];
        emit "ldreq" [Register (R 0); Literal (nosym, !line)];
...
and e_rand =
  ...
  function
    ...
    | <BINOP Lsl, t1, <CONST n>> when fits_offset n ->
      let v1 = e_reg t1 anyreg in
      Shift (reg_of v1, n)
    | t -> e_reg t anyreg
...
and e_addr =
  ...
  function
    ...
    | <OFFSET, t1, <CONST n>> when fits_offset n ->
        let v1 = e_reg t1 anyreg in
        Index (reg_of v1, n)
    | <OFFSET, t1, <BINOP Lsl, t2, <CONST n>>> when fits_offset n ->
        let v1 = e_reg t1 anyreg in
        let v2 = e_reg t2 anyreg in
