  ...
  match s.s_guts with
    ...
    | Assignlist (lhs, rhs) ->
        ...
        let rec check_varlist lh rh =
          ...
          | lhd::ltl -> 
              match rh with
                [] -> ()
              | rhd::rtl -> check_element lhd rhd; check_varlist ltl rtl in
        if List.length lhs <> List.length rhs then sem_error "different lengths of lhs and rhs" []
        else check_varlist lhs rhs
    ...
    | CaseStmt (sel, arms, deflt) ->
        ...
        if not (discrete st) then
          sem_error "expression in case statement must be discrete" [];
...
and check_decl d env = 
  ...
  match d with
    ...
    | ProcDecl (Heading (x, _, _) as heading, body) ->
        ...
        let lab =
          if !level = 0 then sprintf "_$" [fId d.d_tag]
          else sprintf "_$_$" [fId d.d_tag; fStr (gensym ())] in
        d.d_addr <- Global lab;
